<?php
namespace Surething\Muralprod\Block\Product\View\Options\Type;
...
class Select extends \Magento\Catalog\Block\Product\View\Options\AbstractOptions 
{
    ...
    public function getValuesHtml()
    {
        ...
        if ($_option->getType() == \Magento\Catalog\Api\Data\ProductCustomOptionInterface::OPTION_TYPE_DROP_DOWN ||
			...
			if ($_option->getTitle() == "Materials" || $_option->getTitle() == "Finish") {
				foreach ($_option->getValues() as $_value) {
					
					if (!preg_match("/Specialty:/i",$_value->getTitle())) {
						$select->addOption(
							$_value->getOptionTypeId(),
							$_value->getTitle(),
							['price' => $this->pricingHelper->currencyByStore($_value->getPrice(true), $store, false)]
						);
					} elseif ($areaCode == \Magento\Framework\App\Area::AREA_ADMINHTML) {
						$select->addOption(
							$_value->getOptionTypeId(),
							$_value->getTitle(),
							['price' => $this->pricingHelper->currencyByStore($_value->getPrice(true), $store, false)]
